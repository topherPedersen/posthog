...
from posthog.events_postgres import Person, Element, Event, PersonDistinctId, Action
...
from django_clickhouse.database import connections, Database
from django_clickhouse.exceptions import RedisLockTimeoutError
from django_clickhouse.models import ClickHouseSyncModel
from django_clickhouse.query import QuerySet as ClickHouseQuerySet
from django_clickhouse.serializers import Django2ClickHouseModelSerializer
from django_clickhouse.utils import lazy_class_import, exec_multi_arg_func

# logger = logging.getLogger('django-clickhouse')

class QuerySet(ClickHouseQuerySet):
    @property
    def person(self):
    ...
    def filter_by_action(self, action: Action, count_by: str=None, where=None, count=False):
        queryset = self
        filter = Q()
        for step in action.steps.all():
            filter |= Q(event=step.event)
        queryset = self.filter(filter)

        if count_by == 'day':
            queryset = queryset.aggregate(day='toDate(timestamp)', count='count()').group_by('day')
    ...
    def create(self, **kwargs):
        if kwargs.get('team'):
            kwargs['team'] = kwargs['team'].pk
        if kwargs.get('properties') or 'properties' in kwargs:
            properties = kwargs.pop('properties')
            kwargs['property1'] = properties.get('property1')
            kwargs['property2'] = properties.get('property2')
...
class Event(ClickHouseModel):
    team = fields.UInt32Field()
    event = fields.StringField()
    distinct_id = fields.StringField()
    timestamp = fields.DateTimeField()
    ip = fields.StringField()

    property1 = fields.NullableField(fields.StringField())
    property2 = fields.NullableField(fields.StringField())

    engine = engines.MergeTree('timestamp', ('event', 'distinct_id'))
